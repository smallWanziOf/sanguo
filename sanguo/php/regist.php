<?php
	...
	$exist="SELECT user_id FROM sanguo_login WHERE user_name='$name'";
	...
	if($row){
		...
		$sql = "INSERT INTO sanguo_login(user_name,user_pwd,user_power) VALUES ('$name','$pwd',100)";
		$resultSql = mysqli_query($conn, $sql);
		$sqlinster = "INSERT INTO user_sign(user_name) VALUES ('$name')";
		$resultinster = mysqli_query($conn, $sqlinster);
		//查询当前的主键值
		$keysql = "SELECT user_id FROM sanguo_login WHERE user_name='$name'";
		$resultkey = mysqli_query($conn, $keysql);
		$rowkey = mysqli_fetch_assoc($resultkey);
		$id= $rowkey["user_id"];
		//设置order
		$ordersql = "UPDATE sanguo_login SET user_order=$id WHERE user_name='$name'";
